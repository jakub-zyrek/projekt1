...
if (!mysqli_connect_errno()) {
  if (isset($_GET['l'])) {
    $l = $_GET['l'];
    $sql = "SELECT * FROM liczba_odpowiedzi WHERE nazwa = '$id_kat' ORDER BY id DESC LIMIT 10 OFFSET $l ";
  } else {
    $sql = "SELECT * FROM liczba_odpowiedzi WHERE nazwa = '$id_kat' ORDER BY id DESC LIMIT 10";
  }
  
  $zap1 = mysqli_query($polaczenie, $sql);

  $sql1 = "SELECT COUNT(id) AS 'idd' FROM liczba_odpowiedzi WHERE nazwa = '$id_kat'";
    ...
    <div class="container">
      <div class="card">
        <div class="card-header bg-info">
          <h2 class="p-4 text-center">Posty dla kategorii <b><?php echo $_GET['kat']; ?></b></h2>
        </div>
        <div class="card-body">
          <?php
            ...
            echo '<div class="card">';
              echo '<div class="card-header p-4 bg-success-subtle" style="display: flex; justify-content: space-between; flex-wrap: wrap;">';
                echo '<h3 class="col-12 col-md-8">';
                  echo $w['tytul'];
                echo '</h3>';
                echo '<div class="text-end m-md-0 col-4 d-flex" style="width: max-content; margin: auto; align-content: center; justify-content: end;">';
              ...
              echo '<div class="card-body">';
                echo "<img src='".$w['obraz']."' alt='' width='32' height='32' class='rounded-circle me-2'>".$w['nick']." · ";
                if ($w['odp'] == 1) {
              ...
              <a class="btn btn-outline-info dropdown-toggle me-2 mb-3 mb-lg-auto " data-bs-toggle="dropdown" aria-expanded="false">
                ...
                <ul class="dropdown-menu">
                  <?php
                    ...
                    $a = 1;
                    do {
                      echo "<li><a class='dropdown-item' href='posty_k.php?l=$i&a=$a&kat=$id_kat'>$a</a></li>";
                      $i = $i + 10;
                      $a++;
                    } while ($i < $liczba)
                  ?>             
