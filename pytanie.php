...
if (isset($_GET['idpytania'])) {
  ...
  $sql1 = "SELECT *, pytanie.id AS 'idd', uzytkownik.id AS 'uzy_id' FROM pytanie JOIN kategorie ON kategorie.id = pytanie.kategoria_id JOIN uzytkownik ON uzytkownik.id = pytanie.uzytkownik_id WHERE pytanie.id = $id_pytania";
  ...
  <body>
    ...
    <script>
      ...
      function zgloszenie(obiekt, id, uzytkownikk) {
        ...
        opinie.value = opinie.value.replaceAll(",", '%2B');
        plik = "script_PHP/zgloszenie.php?uzytkownik=" + uzytkownikk + "&obiekt=" + obiekt + "&id=" + id + "&opinia=" + opinie.value;
        xml = null;
        try {
          xml = new ActiveXObject("Microsoft.XMLHTTP"); // IE
        } catch (e) {
